...
from models.NASv1.decode import get_new_network
from models.NASv1.erfnet_retrain import *
...
def main():
    ...
    model = Encoder_retrain(num_class, switches_cell)
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            torch.nn.Module.load_state_dict(model, checkpoint['state_dict'], strict=False)
...
def train(train_loader, model, criterion, criterion_exist, optimizer, epoch):
    ...
    for i, (input, target, target_exist) in enumerate(train_loader):
        ...
        if (i + 1) % args.print_freq == 0:
            print(('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t' 'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' 'Data {data_time.val:.3f} ({data_time.avg:.3f})\t' 'Loss {loss.val:.4f} ({loss.avg:.4f})\t' 'Loss_exist {loss_exist.val:.4f} ({loss_exist.avg:.4f})\t'.format(
